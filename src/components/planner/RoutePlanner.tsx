...
export default function RoutePlanner({ defaultCenter }: RoutePlannerProps) {
  ...
  const [status, setStatus] = useState<string | null>(
    'Click a long trail on the map (or in the list) to start connecting.',
  );
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="flex flex-1 min-h-0 flex-col lg:flex-row">
        <aside className="w-full lg:w-80 xl:w-96 shrink-0 border-b lg:border-b-0 lg:border-r border-stone-200 bg-stone-50 flex flex-col min-h-0 max-h-[45vh] lg:max-h-none">
          <div className="p-4 flex flex-col gap-3 min-h-0 flex-1 overflow-hidden">
            <div className="flex gap-2" role="group" aria-label="Build mode">
              <button
                ...
                onClick={() => {
                  setMode('connect');
                  setStatus('Connect mode: click trails to stitch them together.');
                }}
              >
                Connect trails
              </button>
            ...
            {sidebarTab === 'trails' ? (
              ...
              <p className="text-sm text-stone-500">
                No route yet. Switch to Long trails and click one to begin — or click a colored line on the map.
              </p>
            ) : (
              <WaypointList
                waypoints={waypoints}
        ...
        <section className="flex-1 relative min-h-[300px] lg:min-h-0">
          ...
          >
            {mode === 'connect'
              ? highlightedTrailId
                ? `${getTrailById(highlightedTrailId)?.name ?? ''} — click to connect`
                : 'Click a trail to add it to your route'
