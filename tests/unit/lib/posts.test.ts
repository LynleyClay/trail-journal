...
describe('getAllPosts', () => {
  ...
  it('returns empty array when posts.json is empty', () => {
    setupMocks([]);
...
describe('getPublishedPosts', () => {
  ...
  it('returns posts sorted newest-first', () => {
    ...
    setupMocks(posts);
    const [first, second] = getPublishedPosts();
...
describe('getPostBySlug', () => {
  it('returns post with body for a published slug', () => {
    setupMocks([mockPost()], { 'test-post-2026-06': '# Story\n\nThe hike was great.' });
    const post = getPostBySlug('test-post-2026-06');
  ...
  it('returns null for an unpublished post', () => {
    setupMocks([mockPost({ published: false })]);
    expect(getPostBySlug('test-post-2026-06')).toBeNull();
  });
});
